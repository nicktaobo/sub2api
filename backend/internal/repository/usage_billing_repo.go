...
// claimUsageBillingRequest 返回 (claimed, dedupID, error)，dedupID 语义见 claimUsageBillingKey。
func (r *usageBillingRepository) claimUsageBillingRequest(ctx context.Context, tx *sql.Tx, requestID string, apiKeyID int64, requestFingerprint string) (bool, int64, error) {
	var id int64
...
func (r *usageBillingRepository) applyBatchImageBalanceHold(
	...
	// 批量画图冻结/结算/释放路径不写 merchant outbox/ledger，dedupID 用不上，忽略。
	applied, _, err := r.claimUsageBillingRequest(ctx, tx, cmd.RequestID, cmd.APIKeyID, cmd.RequestFingerprint)
	if err != nil {
		return nil, err
	}
	if !applied {
