...
func TestGeminiForwardNative_PoolModeSkipped400PassthroughRealStatus(t *testing.T) {
	...
	// 本 fork 与上游的差异：状态码保真，但**不裸透传上游 body**（会暴露上游厂商/
	// 中转域名），改为按 Google 错误结构重新写出脱敏后的 message —— 与本文件里
	// ForwardAsChatCompletions 池模式的写出口径一致（见
	// TestGeminiForwardAsChatCompletions_PoolMode400KeepsUpstreamMessage）。
	var got map[string]any
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	errObj, ok := got["error"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, geminiSkippedTestUpstreamMsg, errObj["message"], "应回传脱敏后的上游 message")
	require.NotEqual(t, upstreamBody, rec.Body.String(), "不应原样透传上游 body")
}
