...
// testGrokAccountConnection tests a Grok (xAI) account by probing the xAI
// /responses endpoint the same way the live gateway (forwardGrokResponses) does,
// rather than falling through to the Anthropic Messages probe.
func (s *AccountTestService) testGrokAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	// Resolve auth token: OAuth uses access_token, otherwise the api_key credential.
	var token string
	if account.Type == AccountTypeOAuth {
		token = account.GetGrokAccessToken()
		if token == "" {
			return s.sendErrorAndEnd(c, "No Grok access token available (needs reauth)")
		}
	} else {
	...
	// Minimal xAI responses request body.
	...
	// Reuse the production request builder so the URL/host (xAI /responses) and
	// Authorization: Bearer header match the live gateway exactly.
	req, err := buildGrokResponsesRequest(ctx, c, account, body, token)
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden {
			errMsg := fmt.Sprintf("xAI authentication failed (%d), needs reauth: %s", resp.StatusCode, string(body))
			if s.accountRepo != nil {
				_ = s.accountRepo.SetError(ctx, account.ID, errMsg)
			}
			return s.sendErrorAndEnd(c, errMsg)
		}
		return s.sendErrorAndEnd(c, fmt.Sprintf("xAI responses API returned %d: %s", resp.StatusCode, string(body)))
	}

	s.sendEvent(c, TestEvent{Type: "content", Text: "xAI responses probe succeeded"})
