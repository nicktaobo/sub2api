...
func TestLegacyLongContextRule_OnlyGemini(t *testing.T) {
	...
	// 本 fork 终态 8 平台全覆盖：上游只列了自己关心的几个，漏掉 grok 与改名后的国产三家。
	// 这条反例守卫防的是「上游日后给某平台也加旧长上下文规则」——那会让该平台 >200K 的请求
	// 被静默乘 geminiLegacyLongContextMultiplier(2.0)，属直接多收且无告警。
	for _, platform := range []string{
		PlatformOpenAI, PlatformAnthropic, PlatformAntigravity, PlatformComposite,
