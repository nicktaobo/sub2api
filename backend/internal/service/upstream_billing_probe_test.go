...
// ListShadowsByParent 是本地 fork 给 AccountRepository 加的影子账号方法（倍率变更需传播到影子账号）。
// 上游这个 stub 只内嵌接口没实现它，UpdateAccount 走到倍率传播时会命中内嵌的 nil 接口而 panic。
// 实现在基础 stub 上，内嵌它的各个 admin stub 一并覆盖；本组用例不涉及影子账号，返回空即可。
func (r *upstreamBillingProbeAccountRepo) ListShadowsByParent(context.Context, int64) ([]*Account, error) {
	return nil, nil
}
