...
func TestCalculateTokenCostContextTierEnablement(t *testing.T) {
	...
	// 与上游的差异点（自上游移植时按 fork 语义拆开）：上游这条子用例传
	// accountDisabled=false 仍期望 400e-6，即账号级 false 不否决分组开关。
	// 本 fork 保留否决语义，理由见 billing_service.go calculateTokenCost 的
	// ⚠️ 注释：两侧默认值正好相反——groups.long_context_pricing_enabled 在
	// 迁移 221 是 NOT NULL DEFAULT TRUE 且把存量分组全部刷成 TRUE，而 OpenAI 账号的
	// openai_long_context_billing_enabled 缺省为 false。跟随上游语义会让
	// 「分组恒 true × 账号默认 false」这一绝大多数生产组合恒定走长上下文阶梯，
	// 推翻 2026-07-14 拍板的「OpenAI 长上下文计费默认关」，且后台账号开关失效。
