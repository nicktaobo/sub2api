...
func scheduleScenarios() []scheduleScenario {
	...
	return []scheduleScenario{
		...
		},
		{
			...
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
				// fork 定制：fillGeminiCacheWritePrice 给 Gemini 补「缓存写入价 = 标准输入价」，
				// 上游该用例原本断言 cache_write 为 nil。补值只影响模型列表展示：Gemini 用量
				// 从不产生 CacheCreationTokens（extractGeminiUsage 只映射 cachedContentTokenCount
				// 到 CacheReadInputTokens），实际扣费不受影响。两档同为基础输入价——Google
				// 的缓存写入不随长上下文档位加价，故 >200K 档也不乘 2。
				requireTier(t, s.Tiers[0], 0, intPtr(200000), "≤200K", p(1.25e-6), p(10e-6), p(1.25e-6), p(0.3125e-6))
				requireTier(t, s.Tiers[1], 200000, nil, ">200K", p(2.5e-6), p(10e-6), p(1.25e-6), p(0.625e-6))
			},
		},
		{
