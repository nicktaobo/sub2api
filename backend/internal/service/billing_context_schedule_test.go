	...
	return []scheduleScenario{
		...
		},
		{
			...
			check: func(t *testing.T, s *ContextPricingSchedule) {
				...
				// 本用例的目录 fixture（geminiLadderCatalogJSON）显式给了
				// cache_creation_input_token_cost，因此 fork 定制的 fillGeminiCacheWritePrice
				// 在这里是 no-op（它只在 cache_creation ≤ 0 时补值），断言直接跟随上游。
				// fork 那条补值逻辑由下面的「Gemini 目录缺缓存写入价」用例单独守。
		...
		{
			// fork 定制守卫（原挂在上游已删除的「Gemini 旧规则按超出部分计价」用例上）：
			// LiteLLM 价表的 gemini 条目一律没有 cache_creation_input_token_cost，
			// fillGeminiCacheWritePrice 给它补「缓存写入价 = 标准输入价」（Google 对缓存
			// 写入不另收费）。不补的话模型列表「缓存创建价格」列恒为空。
			// 只影响展示：Gemini 用量从不产生 CacheCreationTokens
			// （extractGeminiUsage 只把 cachedContentTokenCount 映射到 CacheReadInputTokens），
			// 实际扣费不受影响。上游没有这段逻辑，合并时不要跟着删。
			name: "Gemini 目录缺缓存写入价时补成标准输入价", model: "gemini-2.5-pro", platform: PlatformGemini, groupPlatform: PlatformGemini,
			group: enabledGroup(PlatformGemini), catalog: geminiCatalogStub(), wantBasis: ContextPricingBasisWholeRequest,
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 1)
				requireTier(t, s.Tiers[0], 0, nil, "", p(1.25e-6), p(10e-6), p(1.25e-6), p(0.3125e-6))
			},
		},
		{
