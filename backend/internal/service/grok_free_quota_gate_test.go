...
// 以下三个复数版方法是本地 fork 给 AccountRepository 加的多平台调度接口（国产五平台走
// 复数版路径）。上游这个 stub 只实现了单数版，缺它们会在 listSchedulableAccounts 里命中
// 内嵌的 nil 接口而 panic。listSchedulableAccounts 按 groupID/RunMode 分派到其中一个，
// 三个都补上，避免调用分支变化时再次踩坑。语义与单数版一致（本用例不按平台过滤）。
func (r *grokFreeQuotaAccountRepoStub) ListSchedulableByPlatforms(context.Context, []string) ([]Account, error) {
	return append([]Account(nil), r.accounts...), nil
}

func (r *grokFreeQuotaAccountRepoStub) ListSchedulableUngroupedByPlatforms(context.Context, []string) ([]Account, error) {
	return append([]Account(nil), r.accounts...), nil
}

func (r *grokFreeQuotaAccountRepoStub) ListSchedulableByGroupIDAndPlatforms(context.Context, int64, []string) ([]Account, error) {
	return append([]Account(nil), r.accounts...), nil
}
