...
// ListSchedulableByPlatforms 等三个复数平台方法是本地 fork 给 AccountRepository 新增的：
// 本地调度器（openai_gateway_scheduling.go listSchedulableAccounts）走的是复数分支，
// 上游 stub 只实现了单平台版本，不补齐会命中内嵌 nil 接口导致 panic。
// 口径与单平台实现保持一致：每次选号只累加一次 selectionCalls。
...
func (r *grokCredentialHandlerRepo) ListSchedulableUngroupedByPlatforms(ctx context.Context, platforms []string) ([]service.Account, error) {
	return r.ListSchedulableByPlatforms(ctx, platforms)
}
